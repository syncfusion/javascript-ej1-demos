...
if (absoluteUrl.match(/^.*(?=(\?_ga))/)) {
	absoluteUrl = absoluteUrl.match(/^.*(?=(\?_ga))/)[0]; // To remove google analytics string
}
...
initControls = function () {
	...
	$.ajax({
        url: 'scripts/l10n/reportdesigner/ej.localetexts.' + $("#culture_switcher").val() + '.min.js',
        async: false
    });
